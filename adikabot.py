...
def init_db():
    ...
    try:
        ...
        # Listings Table Definition (11 Columns Explicit)
        if DATABASE_URL:
            cursor.execute("""
...
def get_listings_by_category(main_category=None, sub_category=None, action_type=None, property_type=None, limit=10, offset=0):
    ...
    try:
        ...
        query = "SELECT id, user_chat_id, user_name, req_type, main_category, sub_category, action_type, property_type, description, status, created_at FROM listings WHERE status = 'pending'"
        params = []
...
def count_listings(main_category=None):
    conn = None
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
...
def add_broker(chat_id, full_name, phone, location):
    ...
    try:
        ...
        cursor = conn.cursor()
        p = get_placeholder()
        query = f"INSERT INTO brokers (chat_id, full_name, phone, location) VALUES ({p}, {p}, {p}, {p})"
        params = (chat_id, full_name, phone, location)
...
BROKER_NAME, BROKER_PHONE, BROKER_LOCATION = range(7, 10)
RESP_MAIN, RESP_ROLE, RESP_PROPERTY, RESP_SUB, RESP_DETAILS, RESP_PRICE, RESP_NEGO, RESP_PHONE, RESP_PHOTO = range(10, 19)
...
async def go_home(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message:
        ...
        await context.bot.send_message(chat_id=update.effective_user.id, text="👇", reply_markup=reply_markup)
    return ConversationHandler.END
...
async def buyer_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    req_id = add_listing(user.id, user.first_name, 'BUY', main_cat, sub_cat, action_type, property_type, desc)
    
    if req_id:
        await update.message.reply_text(
            f"✅ **ጥያቄዎ ተመዝግቧል!** (#REQ-{req_id})\n\n📌 ጥያቄዎ በ'📋 የፈላጊዎች ዝርዝር' ውስጥ ይታያል።",
            reply_markup=ReplyKeyboardMarkup(MAIN_KEYBOARD, resize_keyboard=True)
        )
    return ConversationHandler.END

# ==============================================================================
# 8. VIEW REQUESTS (SAFE DICT-BASED)
# ==============================================================================
ITEMS_PER_PAGE = 5
...
async def view_requests(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not broker:
        await update.message.reply_text(
            "⛔ ይህን ገጽ ማየት የሚችሉት የተመዘገቡ አቅራቢዎች ብቻ ናቸው!\n\n'📝 እንደ አቅራቢ መመዝገብ' ይጫኑ።",
            reply_markup=ReplyKeyboardMarkup(MAIN_KEYBOARD, resize_keyboard=True)
        )
        return
...
async def show_requests_page(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        for listing in listings:
            ...
            text += f"{icon} **#{listing_id}**\n📝 {description}\n────────────────────\n"
        
        keyboard = []
        for listing in listings:
            l_id = listing.get('id')
            u_id = listing.get('user_chat_id')
            m_cat = listing.get('main_category')
            keyboard.append([InlineKeyboardButton(f"✅ አለኝ - #{l_id}", callback_data=f"item_resp_{l_id}_{u_id}_{m_cat}")])
...
async def resp_role_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('target_cat') == "car":
        await query.edit_message_text("🚘 **የመኪናውን ሞዴል እና ሁኔታ ያስገቡ፦**")
        return RESP_DETAILS
    else:
        keyboard = [[InlineKeyboardButton(ptype, callback_data=f"resp_prop_{ptype}")] for ptype in PROPERTY_TYPES]
        keyboard.append([InlineKeyboardButton("🏠 ዋና ገጽ", callback_data="flow_home")])
        await query.edit_message_text("🏠 **የቤት/ቦታ አይነት ይምረጡ፦**", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def resp_property_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    context.user_data['resp_property'] = query.data.replace("resp_prop_", "")
    
    keyboard = [[InlineKeyboardButton(htype, callback_data=f"resp_htype_{htype}")] for htype in HOUSE_TYPES]
    keyboard.append([InlineKeyboardButton("🏠 ዋና ገጽ", callback_data="flow_home")])
    await query.edit_message_text("🏠 **የቤት አይነት ይምረጡ፦**", reply_markup=InlineKeyboardMarkup(keyboard))
    return RESP_SUB
...
async def resp_htype_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    context.user_data['resp_htype'] = query.data.replace("resp_htype_", "")
    await query.edit_message_text("📍 **አካባቢ እና ዝርዝር መረጃ ያስገቡ፦**")
    return RESP_DETAILS
...
async def resp_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("💰 **ዋጋ ያስገቡ፦**", reply_markup=ReplyKeyboardMarkup([["🏠 ዋና ገጽ"]], resize_keyboard=True))
    return RESP_PRICE
...
async def resp_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🔄 **የዋጋ ድርድር ሁኔታ፦**", reply_markup=InlineKeyboardMarkup(keyboard))
    return RESP_NEGO
...
async def resp_nego(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("📞 **የስልክ ቁጥር ያስገቡ፦**")
    return RESP_PHONE
...
async def resp_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📸 **የንብረቱን ፎቶ ይላኩ፦**", reply_markup=ReplyKeyboardMarkup([["🏠 ዋና ገጽ"]], resize_keyboard=True))
    return RESP_PHOTO
...
async def resp_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    desc = (
        ...
        f"🎭 ሚና: {context.user_data.get('resp_role')}\n"
        f"📝 {context.user_data.get('resp_details')}\n"
        f"💰 {context.user_data.get('resp_price')} ብር ({context.user_data.get('resp_nego')})\n"
        f"📞 {context.user_data.get('resp_phone')}\n"
        f"👤 @{responder.username if responder.username else responder.first_name}"
    )
    
    try:
        await context.bot.send_photo(chat_id=target_user_id, photo=photo_id, caption=desc, parse_mode="Markdown")
        update_listing_status(int(req_id), 'responded')
        await update.message.reply_text("✅ **መረጃዎች ለፈላጊው ተልከዋል!**", reply_markup=ReplyKeyboardMarkup(MAIN_KEYBOARD, resize_keyboard=True))
    except Exception as e:
        logger.error(f"Error sending response: {e}")
        await update.message.reply_text("❌ ምላሽ ሲላክ ስህተት ተከስቷል።")
...
async def broker_reg_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data.clear()
    await update.message.reply_text("📝 **እንደ አቅራቢ/ደላላ መመዝገብ**\n\n1️⃣ ሙሉ ስምዎን ያስገቡ፦", reply_markup=ReplyKeyboardMarkup([["🏠 ዋና ገጽ"]], resize_keyboard=True))
    return BROKER_NAME
...
async def broker_reg_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton(loc, callback_data=f"broker_loc_{loc}")] for loc in LOCATIONS]
    keyboard.append([InlineKeyboardButton("🏠 ዋና ገጽ", callback_data="flow_home")])
    await update.message.reply_text("3️⃣ የሚሰሩበትን አካባቢ ይምረጡ፦", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def broker_reg_location(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if broker_id:
        await query.edit_message_text("✅ **በስኬት ተመዝግበዋል!**\n\nአሁን '📋 የፈላጊዎች ዝርዝር' በመጠቀም ጥያቄዎችን ማየት ይችላሉ!")
    else:
        await query.edit_message_text("❌ አስቀድመው ተመዝግበዋል!")
    return ConversationHandler.END
