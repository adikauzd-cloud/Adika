...
# ==============================================================================
(
    BUYER_MAIN, BUYER_SUB, BUYER_ACTION, BUYER_DETAILS, BUYER_PHONE,
...
async def buyer_category_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton(opt, callback_data=f"{prefix}{opt}")] for opt in options]
    keyboard.append([InlineKeyboardButton("🏠 ዋና ገጽ", callback_data="flow_home")])
    
    await query.edit_message_text(
        f"📌 **የ{sub_category_name(cat)} ንኡስ ምድብ ይምረጡ፦**",
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode="Markdown",
    )
    return BUYER_SUB

def sub_category_name(cat: str) -> str:
    """Get sub-category name in Amharic"""
    names = {
        "car": "መኪና",
        "house": "ቤት/ቦታ",
        "commercial": "የሥራ ቦታ"
    }
    return names.get(cat, "")
...
async def buyer_action_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "📝 **እባክዎን የሚፈልጉትን ንብረት ዝርዝር መግለጫ ያስገቡ፦**\n\n"
        "💡 *ምሳሌ፦ አካባቢ፣ የክፍል ብዛት፣ የበጀት መጠን ወዘተ.*",
        parse_mode="Markdown"
    )
...
async def buyer_details_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(text.strip()) < 5:
        await safe_send_message(
            update, context,
            "⚠️ እባክዎን ቢያንስ 5 ፊደላት ያለው ዝርዝር መግለጫ ያስገቡ፦"
        )
        return BUYER_DETAILS
...
async def buyer_phone_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle buyer phone input and save listing"""
    text = update.message.text
    ...
    # Build full description with phone
    ...
    if listing_id:
        # Notify brokers
        notification_text = (
            f"📢 **አዲስ የፍላጎት ጥያቄ (#{listing_id})**\n\n"
            f"📌 **ምድብ:** {listing_data['main_category']}\n"
            f"🔹 **ንኡስ ምድብ:** {listing_data['sub_type']}\n"
            f"🔹 **ዓይነት:** {listing_data['action_type']}\n"
            f"📝 **መግለጫ:**\n{description}\n"
            f"📞 **ስልክ:** {phone}"
        )
...
async def seller_category_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton(opt, callback_data=f"flow_sell_sub_{opt}")] for opt in options]
    keyboard.append([InlineKeyboardButton("🏠 ዋና ገጽ", callback_data="flow_home")])
...
async def seller_action_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "📝 **እባክዎን የንብረቱን ዝርዝር መግለጫ ያስገቡ፦**\n\n"
        "💡 *ምሳሌ፦ አካባቢ፣ የክፍል ብዛት፣ ሁኔታ ወዘተ.*",
        parse_mode="Markdown"
    )
...
async def seller_details_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(text.strip()) < 5:
        await safe_send_message(update, context, "⚠️ እባክዎን ቢያንስ 5 ፊደላት ያለው መግለጫ ያስገቡ፦")
        return SELLER_DETAILS
    
    context.user_data['description'] = sanitize_input(text)
    await safe_send_message(update, context, "💰 **የሚሸጡበትን/ሚያከራዩበትን ዋጋ ያስገቡ፦**")
    return SELLER_PRICE
...
async def seller_price_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not validate_price(text):
        await safe_send_message(update, context, "⚠️ እባክዎን ትክክለኛ ዋጋ ያስገቡ (ቁጥር ብቻ)፦")
        return SELLER_PRICE
    
    context.user_data['price'] = text
    await safe_send_message(update, context, "📱 **እባክዎን ስልክ ቁጥርዎን ያስገቡ፦**")
    return SELLER_PHONE
...
async def broker_reg_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not validate_phone(update.message.text):
        await safe_send_message(
            update, context,
            "⚠️ ትክክለኛ የስልክ ቁጥር ያስገቡ። (ለምሳሌ፦ 0911223344)"
        )
        return BROKER_PHONE
