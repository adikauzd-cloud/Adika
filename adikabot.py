...
# የስልክ ቁጥር እና Contact Share ማድረጊያ ኪቦርድ
...
def extract_phone_and_username(update: Update) -> tuple[Optional[str], str]:
    """ከMessage ወይም Contact ላይ ስልክ እና Username መለያ"""
...
async def buyer_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "📞 **እርስዎን የሚያገኙበትን የስልክ ቁጥር ያስገቡ ወይም ከታች ያለውን ባተን ተጭነው ያጋሩ፦**",
        parse_mode="Markdown",
...
async def buyer_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = update.effective_user
    phone, username = extract_phone_and_username(update)
    
    if not phone:
        await update.message.reply_text(
            "❌ **ትክክለኛ የስልክ ቁጥር አልተገኘም!**\nእባክዎ ቁጥር በጽሁፍ ያስገቡ ወይም ከታች ያለውን '📲 ስልክ ቁጥር አጋራ' የሚለውን ይጫኑ።",
            reply_markup=PHONE_KEYBOARD
        )
        return BUYER_PHONE
    ...
    item_type = sub_cat if main_cat == "car" else f"{prop_type} ({prop_subtype})"
    ...
    user_display_name = f"{user.first_name} ({username})"
    req_id = add_listing(user.id, user_display_name, 'BUY', main_cat, sub_cat, action_type, prop_subtype, full_desc)
    ...
    if req_id:
        ...
        await update.message.reply_text(
            "❌ ጥያቄዎን መመዝገብ አልተቻለም። እባክዎ እንደገና ይሞክሩ።",
            reply_markup=ReplyKeyboardMarkup(MAIN_KEYBOARD, resize_keyboard=True)
        )
...
async def seller_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "📞 **የስልክ ቁጥርዎን ያስገቡ ወይም ከታች ያለውን ባተን ተጭነው ያጋሩ፦**",
        reply_markup=PHONE_KEYBOARD
    )
    return SELLER_PHONE
...
async def seller_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not phone:
        await update.message.reply_text(
            "❌ **ትክክለኛ የስልክ ቁጥር አልተገኘም!**\nእባክዎ ቁጥር ያስገቡ ወይም ከታች ያለውን '📲 ስልክ ቁጥር አጋራ' የሚለውን ይጫኑ።",
            reply_markup=PHONE_KEYBOARD
        )
        return SELLER_PHONE
...
async def seller_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    add_listing(user.id, user.first_name, 'SELL', context.user_data.get('main_category'), '', context.user_data.get('action_type'), context.user_data.get('property_type', ''), desc)
...
async def broker_reg_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "2️⃣ **የስልክ ቁጥርዎን ያስገቡ ወይም ከታች ያለውን ባተን ተጭነው ያጋሩ፦**",
        reply_markup=PHONE_KEYBOARD
    )
    return BROKER_PHONE
...
async def broker_reg_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['broker_username'] = username
...
async def broker_reg_nid_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    username = context.user_data.get('broker_username', 'የለውም')
...
def main():
    ...
    phone_input_filter = (filters.CONTACT | filters.TEXT) & ~filters.COMMAND
    ...
    buyer_conv = ConversationHandler(
        ...
        states={
            ...
            BUYER_PHONE: [MessageHandler(phone_input_filter, buyer_phone), cancel_message_handler],
        },
    ...
    seller_conv = ConversationHandler(
        ...
        states={
            ...
            SELLER_PHONE: [MessageHandler(phone_input_filter, seller_phone), cancel_message_handler],
    ...
    broker_conv = ConversationHandler(
        ...
        states={
            ...
            BROKER_PHONE: [MessageHandler(phone_input_filter, broker_reg_phone), cancel_message_handler],
